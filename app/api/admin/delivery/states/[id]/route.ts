...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  const { data, error } = await supabase.from("states").update({ name }).eq("id", id).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ data: { id: data.id, countryId: data.country_id, name: data.name } });
}
