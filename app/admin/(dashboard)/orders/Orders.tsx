...
const ItemTimeline = ({ item }: { item: OrderItem }) => {
    ...
    return (
      <div className="flex items-center gap-2 mt-1">
        ...
        <span
          className={cn(
            satoshi.className,
            "text-[0.75rem] text-red-400 font-medium",
          )}
        >
          Returned
        </span>
  ...
  return (
    <div className="flex items-center gap-0 mt-1">
          ...
          <div key={s} className="flex items-center">
            ...
            {i < ITEM_STATUSES.length - 1 && (
              <div
                className={cn(
                  "w-4 h-px",
                  done && i < idx ? "bg-[#6EC93E]" : "bg-[#E5E7EB]",
                )}
              />
            )}
          </div>
      ...
      <span
        className={cn(satoshi.className, "ml-2 text-[0.75rem] text-[#9CA3AF]")}
      >
...
const ComboChips = ({ combo }: { combo: Record<string, string> }) => {
  ...
  if (entries.length === 0)
    return (
      <span className={cn(satoshi.className, "text-[0.75rem] text-[#9CA3AF]")}>
        No variants
      </span>
    );
  return (
    <div className="flex flex-wrap gap-1">
      {entries.map(([k, v]) => (
        <span
          ...
          className={cn(
            satoshi.className,
            "px-2 py-0.5 bg-[#F3F4F6] rounded-full text-[0.75rem] text-[#374151]",
          )}
        >
          {k}: {v}
        </span>
      ...
      >
        ...
        <div className="flex flex-col gap-3">
          <div>
            <label
              className={cn(
                satoshi.className,
                "text-[0.8125rem] font-medium text-[#374151] mb-1.5 block",
              )}
            >
              Amount refunded (₦)
            </label>
            <input
              ...
              className={cn(
                satoshi.className,
                "w-full px-4 py-2.5 rounded-xl border border-[#E5E7EB] text-[0.875rem] text-[#374151] focus:outline-none focus:border-[#6EC93E]",
              )}
            />
          </div>
          <div>
            ...
            <textarea
              ...
              className={cn(
                satoshi.className,
                "w-full px-4 py-2.5 rounded-xl border border-[#E5E7EB] text-[0.875rem] text-[#374151] focus:outline-none focus:border-[#6EC93E] resize-none",
              )}
            />
          </div>
        </div>
        <div className="flex gap-3 pt-1">
          <Button
            ...
            className={cn(
              satoshi.className,
              "flex-1 py-2! px-0! rounded-xl text-[0.875rem]",
            )}
          >
            Cancel
          </Button>
          <Button
            ...
            className={cn(
              satoshi.className,
              "flex-1 py-2! px-0! rounded-xl bg-red-500 hover:bg-red-600 text-white text-[0.875rem] font-semibold",
            )}
          >
            Record Refund
          </Button>
...
const OrderDrawer = ({
  ...
  const {
    markPackaged,
    markLogisticsReady,
    markAllLogisticsReady,
    refundItem,
    refundOrder,
  } = useOrderStore();
  const [busyId, setBusyId] = useState<string | null>(null);
  const [markingAll, setMarkingAll] = useState(false);
  const [refundItemTarget, setRefundItemTarget] = useState<OrderItem | null>(
    null,
  );
  const [refundOrderOpen, setRefundOrderOpen] = useState(false);

  const handleMarkPackaged = useCallback(
  ...
  const handleMarkLogisticsReady = useCallback(
    async (item: OrderItem) => {
  ...
  const hasEligibleForMarkAll = order.items.some(
    (i) =>
      i.status === "pending" ||
      (i.status === "packaged" && !i.logisticsReady),
  );
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-b border-[#E5E7EB] shrink-0">
          ...
          <div className="flex-1 min-w-0">
            <p
              className={cn(
                poppins.className,
                "text-[0.9375rem] font-semibold text-[#111827] font-mono",
              )}
            >
              {order.orderRef}
            </p>
            <p
              className={cn(satoshi.className, "text-[0.75rem] text-[#9CA3AF]")}
            >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-5">
            ...
            {[
              ...
              <div key={label} className="bg-[#F9FAFB] rounded-xl px-4 py-3">
                ...
                <p
                  className={cn(
                    satoshi.className,
                    "text-[0.875rem] font-semibold text-[#374151] mt-0.5",
                  )}
                >
                  {value}
                </p>
          ...
          <div className="bg-[#F9FAFB] rounded-xl px-4 py-3">
            <p
              className={cn(
                satoshi.className,
                "text-[0.75rem] text-[#9CA3AF] mb-0.5",
              )}
            >
              Delivery address
            </p>
            ...
            {order.userCityName && (
              <p
                className={cn(
                  satoshi.className,
                  "text-[0.75rem] text-[#9CA3AF] mt-0.5",
                )}
              >
                {order.userCityName}
                {order.userStateName ? `, ${order.userStateName}` : ""}
              </p>
            )}
          </div>
          ...
          {canWrite && hasEligibleForMarkAll && !isFullyRefunded && (
            <div className="rounded-xl border border-[#6EC93E]/30 bg-[#6EC93E]/5 p-4 flex items-center justify-between gap-3">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p
                    className={cn(
                      satoshi.className,
                      "text-[0.875rem] font-semibold text-[#374151]",
                    )}
                  >
                    Mark all logistics-ready
                  </p>
                  <p
                    className={cn(
                      satoshi.className,
                      "text-[0.8125rem] text-[#9CA3AF]",
                    )}
                  >
                    Advances pending items to packaged + flags all for logistics
                    pickup
                  </p>
                </div>
              </div>
              <Button
                ...
                className={cn(
                  satoshi.className,
                  "shrink-0 px-4 py-2 rounded-lg bg-[#6EC93E] hover:bg-[#5cb535] text-white text-[0.8125rem] font-semibold",
                )}
              >
                Mark all
              </Button>
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p
                    className={cn(
                      satoshi.className,
                      "text-[0.875rem] font-semibold text-red-700",
                    )}
                  >
                    Refund entire order
                  </p>
                  <p
                    className={cn(
                      satoshi.className,
                      "text-[0.8125rem] text-red-400",
                    )}
                  >
                    Marks all items returned and order as refunded
                  </p>
                </div>
              </div>
              <button
                ...
                className={cn(
                  satoshi.className,
                  "shrink-0 px-3 py-1.5 rounded-lg text-[0.8125rem] font-semibold text-red-600 border border-red-300 hover:bg-red-100 transition-colors",
                )}
              >
                Refund
              </button>
          ...
          <div>
            <p
              className={cn(
                poppins.className,
                "text-[0.875rem] font-semibold text-[#111827] mb-3",
              )}
            >
              Order Items
            </p>
            <div className="flex flex-col gap-3">
              {order.items.map((item) => {
                ...
                const canMarkLR =
                  ...
                  !item.logisticsReady;
                const isLRDone =
                  item.logisticsReady && item.status === "packaged";
                const canRefund =
                  canWrite &&
                    ...
                    <div className="flex items-start gap-3 p-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-0.5">
                          <ComboChips combo={item.variantCombo} />
                        </div>
                        <p
                          className={cn(
                            satoshi.className,
                      ...
                      <div className="px-4 pb-3 flex flex-wrap gap-2">
                        {canPackage && (
                          <button
                            ...
                            className={cn(
                              satoshi.className,
                              "flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[0.8125rem] font-medium bg-[#111827] text-white hover:bg-[#1f2937] transition-colors disabled:opacity-50",
                            )}
                          >
                            {busyId === item.id ? (
                              <Icon
                        ...
                        {canMarkLR && (
                          <button
                            ...
                            className={cn(
                              satoshi.className,
                              "flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[0.8125rem] font-medium bg-[#6EC93E] text-white hover:bg-[#5cb535] transition-colors disabled:opacity-50",
                            )}
                          >
                            {busyId === item.id + "_lr" ? (
                              <Icon
                        ...
                        {isLRDone && (
                          <span
                            className={cn(
                              satoshi.className,
                              "flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[0.8125rem] font-medium bg-[#6EC93E]/10 text-[#3a7a1e]",
                            )}
                          >
                            <Icon
                        ...
                        {item.status === "enroute" && (
                          <span
                            className={cn(
                              satoshi.className,
                              "flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[0.8125rem] bg-blue-50 text-blue-600",
                            )}
                          >
                            <Icon
                        ...
                        {canRefund && (
                          <button
                            ...
                            className={cn(
                              satoshi.className,
                              "flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[0.8125rem] font-medium text-red-600 border border-red-200 hover:bg-red-50 transition-colors",
                            )}
                          >
                            <Icon
        ...
        {refundItemTarget && (
          <RefundModal
            ...
            onConfirm={(amount, notes) =>
              refundItem(order.id, refundItemTarget.id, amount, notes)
            }
...
const Orders = () => {
  ...
  const handleSearch = useCallback(
    (value: string | number) => {
      ...
      fetchOrders(1, String(value) || undefined);
    },
    [fetchOrders],
  );
    ...
    () => [
      {
        ...
        customTableBody: (r: OrderRecord) => (
          <span
            className={cn(
              satoshi.className,
              "text-[0.875rem] font-semibold text-[#111827] font-mono",
            )}
          >
            {r.orderRef}
          </span>
      ...
      },
      {
        ...
        customTableBody: (r: OrderRecord) => (
          ...
          >
            {r.userCityName ?? "—"}
            {r.userStateName ? `, ${r.userStateName}` : ""}
          </p>
        ),
      },
      {
      ...
      },
      {
        ...
        customTableBody: (r: OrderRecord) => (
          <span
            className={cn(
              satoshi.className,
              "text-[0.8125rem] text-[#9CA3AF] whitespace-nowrap",
            )}
          >
            {fmt(r.createdAt)}
          </span>
